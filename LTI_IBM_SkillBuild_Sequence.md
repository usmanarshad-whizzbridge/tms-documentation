# LTI Restoration Methodology - IBM SkillsBuild Integration

**Document Version:** 1.0
**Date:** January 20, 2026
**Prepared for:** IBM SkillBuild
...
The LTI Restoration Methodology describes how the TMS (Training Management System) platform integrates with IBM SkillsBuild using LTI 1.3 standards to launch courses and restore (fetch) learner grades and progress data. This bidirectional integration enables seamless course delivery and comprehensive learning analytics.
...
|---------|-----------|-------|---------|
| **AGS (Grades)** | IBM → TMS | **PUSH** | IBM sends scores TO TMS when learners complete activities |
| **NRPS (Roster)** | IBM ← TMS | **PULL** | IBM fetches enrolled learners FROM TMS (optional) |

The integration consists of two primary phases:

### Phase 1: Launch Flow
- Learner initiates course from TMS platform
- Secure OIDC authentication handshake with IBM SkillsBuild
- Learner accesses IBM course content
- Auto-enrollment in external course

### Phase 2: Restoration Flow (Data Synchronization)

**IBM → TMS (Push Model - Primary)**
- **AGS (Assignment and Grade Services)**: IBM **pushes** grades and scores TO TMS when learners complete activities
- TMS acts as **AGS Provider** (receives grades)
- Automatic update of learner enrollment records (`self_courses`)

**IBM ← TMS (Pull Model - Optional)**
- **NRPS (Names and Role Provisioning Services)**: IBM **fetches** learner roster FROM TMS
- TMS acts as **NRPS Provider** (provides roster data)
- IBM can query enrolled members and their basic progress data
...
1. LAUNCH PHASE (User Initiates)
   Learner → TMS → IBM SkillsBuild

2. LEARNING PHASE
   Learner completes activities in IBM SkillsBuild

3. GRADE RESTORATION (Automatic - Push Model)
   IBM → TMS (POST /api/v1/lti/ags/lineitems/{id}/scores)
   ├─ IBM creates line items
   ├─ IBM posts scores when learner completes
   └─ TMS auto-updates self_courses table

4. ROSTER SYNC (Optional - Pull Model)
   IBM ← TMS (GET /api/v1/lti/nrps/contexts/{id}/memberships)
   └─ IBM fetches enrolled learner list from TMS
```
...
| Grades Fetcher | `app/services/lti/grades_fetcher.rb` | AGS service for retrieving grades from IBM |
| Progress Fetcher | `app/services/lti/progress_fetcher.rb` | NRPS service for retrieving progress from IBM |
...
| ID Token Builder | `app/services/lti/id_token_builder.rb` | Constructs JWT tokens for IBM |
| Token Fetcher | `app/services/lti/token_fetcher.rb` | OAuth 2.0 token management |

### Supporting Models

- **LtiTool**: IBM SkillsBuild tool configuration
...
```
┌─────────┐      ┌──────────┐      ┌─────────┐      ┌──────────────┐
│ Learner │      │React App │      │TMS API  │      │IBM SkillsBuild│
└────┬────┘      └────┬─────┘      └────┬────┘      └──────┬───────┘
     │                │                  │                  │
     ...
     │                │                  │    (if not enrolled)
     │                │                  │                  │
     │                │ 5. Redirect to IBM Login URL        │
     │                │    (with state, nonce, login_hint)  │
     │                │<─────────────────┤                  │
     │                │                                     │
     │ 6. Browser redirect to IBM                           │
     ├────────────────┼─────────────────┼─────────────────>│
     │                │                  │                  │
     │                │                  │ 7. IBM OIDC Auth Callback
     │                │                  │    GET /lti/authorize
     │                │                  │<─────────────────┤
     │                │                  │                  │
     │                │                  │ 8. Validate session
     ...
     │                │                  ├─────────────────>│
     │                │                  │                  │
     │ 11. IBM course loads in new tab                │
     │<──────────────────────────────────┼──────────────────┤
     │                │                  │                  │
     ...
     │                │                  │                  │
╔════════════════════════════════════════════════════════════════╗
║  RESTORATION PHASE: IBM Pushes Grades to TMS                 ║
╚════════════════════════════════════════════════════════════════╝
     │                │                  │                  │
     │                │                  │ 13. IBM creates line item (assignment)
     │                │                  │     Request OAuth token
     │                │                  │     POST /lti/token
     │                │                  │<─────────────────┤
     │                │                  │                  │
...
TMS generates an auto-submitting HTML form that POSTs to IBM with:
...
IBM validates the ID token and loads the course for the learner.
...
#### Purpose
IBM SkillsBuild **pushes** assignment/assessment grades to TMS automatically when learners complete activities. TMS acts as an AGS Provider (receives grades), not a consumer (fetches grades).

#### Step 13-14: IBM Requests OAuth Token
When IBM needs to create a line item or post a score, it requests an access token from TMS:
```
POST /lti/token
...
#### Step 15-16: IBM Creates Line Item
IBM creates an assignment (line item) on TMS:
```
POST /api/v1/lti/ags/contexts/{context_id}/lineitems
...
Learner completes an assessment or activity in IBM SkillsBuild.

#### Step 18-19: IBM Requests Token for Score Posting
IBM requests another access token with score scope.

#### Step 20-21: IBM Pushes Score to TMS
IBM posts the learner's score to TMS:
```
...
### NRPS - IBM Fetches Member Roster from TMS (Optional)

#### Purpose
IBM SkillsBuild can **fetch** (pull) the learner roster from TMS to understand who is enrolled in a course. This allows IBM to display enrolled learners and potentially sync rosters.

**Note**: This is optional and depends on whether IBM SkillsBuild requests roster information.

#### How It Works

When IBM needs roster information, it:
...
**Direction**: IBM → TMS (IBM pulls roster FROM TMS)
...
2. **AGS Provider Endpoints**: Receive grades FROM IBM (IBM calls these)
3. **NRPS Provider Endpoints**: Provide roster TO IBM (IBM calls these)
4. **Consumer Endpoints**: Fetch data from IBM (TMS admins call these - optional/testing)
...
**Response:** Redirects to IBM login URL
...
### AGS Provider Endpoints (IBM Calls These)

These endpoints are called BY IBM to push grades TO TMS. They require OAuth authentication with Bearer tokens issued by TMS.
...
**Called by**: IBM SkillsBuild
**Purpose**: IBM fetches list of line items it previously created
...
**Called by**: IBM SkillsBuild
**Purpose**: IBM fetches results/scores it previously posted
...
These endpoints allow TMS administrators to manually fetch grades FROM IBM. These are optional and primarily for testing.

#### Fetch All Grades from IBM
```
...
**Purpose**: Manually pull all grades from IBM (for testing/debugging)

Implementation: `app/controllers/api/v1/lti/grades_controller.rb`

---

#### Fetch Progress from IBM
```
...
**Called by**: TMS administrators
**Purpose**: Manually pull learner progress from IBM (for testing/debugging)

Implementation: `app/controllers/api/v1/lti/progress_controller.rb`

---

### NRPS Provider Endpoints (IBM Calls These)

These endpoints are called BY IBM to fetch roster information FROM TMS.
...
**Called by**: IBM SkillsBuild
**Purpose**: IBM fetches the list of enrolled learners from TMS
...
- **Audience**: IBM SkillsBuild client ID
...
#### lti_tools
Stores IBM SkillsBuild tool registration.

```ruby
{
  id: integer,
  name: string,                    # "IBM SkillsBuild"
  client_id: string,               # OAuth client ID
  platform_public_jwks_url: string, # Platform's JWKS endpoint
  login_url: string,               # IBM OIDC login URL
  launch_url: string,              # IBM launch endpoint
  public_jwk: text,                # IBM's public key (JWK format)
...
```ruby
{
  ...
  lti_tool_id: integer,            # Foreign key to lti_tools
  provider: string,                # "IBM SkillsBuild"
  external_course_id: string,      # IBM course UUID (exportUuid)
  ...
  external_metadata: jsonb,        # Extra metadata from IBM
  created_at: datetime,
  updated_at: datetime
}
```
...
#### lti_line_items
Cached assignment/assessment data from IBM.
...
```ruby
{
  ...
  context_id: string,              # Course ID
  lineitem_url: string,            # IBM line item URL
  resource_link_id: string,        # Associated resource
  resource_id: string,             # IBM resource ID
...
3. **`app/controllers/api/v1/lti/ags/results_controller.rb`**
   - Provides read access to results for IBM
   - IBM can query scores it previously posted
...
#### NRPS Provider Flow (Provide Roster to IBM)

1. **`app/controllers/api/v1/lti/nrps/memberships_controller.rb`**
   - Provides course membership roster to IBM
...
#### AGS/NRPS Consumer Flow (Optional - TMS pulls from IBM)
...
- **Tool**: The external application (IBM SkillsBuild)
...
## Summary for IBM SkillBuild

### Key Points

1. **Grade Restoration is PUSH-based**
   - IBM SkillsBuild **pushes** grades TO TMS automatically
   - TMS does NOT fetch grades FROM IBM
   - Endpoint: `POST /api/v1/lti/ags/lineitems/{lineitem_id}/scores`

2. **Workflow**
   - Learner launches course from TMS → IBM SkillsBuild
   - Learner completes activity in IBM
   - IBM automatically posts score to TMS
   - TMS updates learner's enrollment record (`self_courses`)

3. **OAuth Authentication**
   - IBM requests access token from TMS: `POST /lti/token`
   - IBM includes token in score POST: `Authorization: Bearer {token}`
...
6. **Roster Access (Optional)**
   - If IBM needs roster data, it can fetch from TMS
